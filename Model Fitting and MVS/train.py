...
def train_sample(sample, detailed_summary=False):
    ...
    # To ensure the sample can be trained using CUDA
    depth_gt = tocuda(sample["depth"])
    mask = tocuda(sample["mask"])

    outputs = model(tocuda(sample["imgs"]), tocuda(sample["proj_matrices"]), tocuda(sample["depth_values"]))
...
def test_sample(sample, detailed_summary=True):
    ...
    # To ensure the sample can be tested using CUDA
    depth_gt = tocuda(sample["depth"])
    mask = tocuda(sample["mask"])
