...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
  ...
  initial_length = points3D.shape[0]
  initial_length_fixed = points3D.shape[0]
  points3D = np.append(points3D, new_points3D, 0)
  number_new_points = new_points3D.shape[0]

  for im_name in corrs:
    kp_idx = corrs[im_name]
    num_points = kp_idx.shape[0]
    if num_points == number_new_points:
      global_3d_idx = np.linspace(initial_length_fixed, initial_length_fixed + num_points - 1, num_points, dtype = np.int32)
      images[im_name].Add3DCorrs(kp_idx, global_3d_idx)
    else:
      global_3d_idx = np.linspace(initial_length, initial_length + num_points - 1, num_points, dtype = np.int32)
      images[im_name].Add3DCorrs(kp_idx, global_3d_idx)
      initial_length += num_points
